...
/**
 * check_elf - Checks whether the file is an ELF file or not
 * @e_ident: points to the array that contains the ELF magic numbers.
 *
 * Description: if file is not an ELF file - exit with the code 98.
 */
void check_elf(unsigned char *e_ident)
{
	int num_index;

		for (num_index = 0; num_index < 4; num_index++)
	{
		if (e_ident[num_index] != 127 &&
		    e_ident[num_index] != 'E' &&
		    e_ident[num_index] != 'L' &&
		    e_ident[num_index] != 'F')
		{
...
void print_magic(unsigned char *e_ident)
{
	...
	for (num_index = 0; num_index < EI_NIDENT; num_index++)
	{
		printf("%02x", e_ident[num_index]);

		if (num_index == EI_NIDENT - 1)
			printf("\n");
		else
			printf(" ");
	}
...
/**
 * print_class - this prints class of the ELF header.
 *
 * @e_ident: points to the array that contains the ELF class.
 */
void print_class(unsigned char *e_ident)
{
	printf("  Class:                             ");
 ...
 *
 * @e_ident: this points to the array containing the ELF ABI version.
 */
void print_abi(unsigned char *e_ident)
{
 ...
 *
 * Description: If file cannot be closed - exit with the code 98.
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
